...
let proc_stat_freq_option =
  let doc =
    "Set the interval, in seconds, at which the process status and the peak \
     ...
     it, on Linux, OSX and Windows, the peak is the maximum over samples of \
     the RSS less the ring: only the current RSS can be decomposed that way, \
     so a shorter interval lowers the chance of missing a transient peak at \
     the cost of more sampling overhead, a genuine accuracy/overhead tradeoff. \
