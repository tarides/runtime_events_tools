...
type trace =
  { doms: Trace.Thread.t array
  ; file : Trace.t }
let create ~filename =
  let file = Trace.create_for_file ~base_time:None ~filename:filename in
  let doms =
    let max_doms = 128 in
    Array.init max_doms (fun i ->
        (* Use a different pid for each domain *)
        Trace.allocate_thread file ~pid:i
...
let emit trace evt =
  ...
  | SpanBegin | SpanEnd ->
     let write = 
       if evt.kind = SpanBegin
       then Trace.write_duration_begin
       else Trace.write_duration_end in
     write trace.file ~args:[] ~thread ~category ~name ~time
  | Counter value ->
     Trace.write_counter
       trace.file ~thread ~category ~name ~time ~args:[ ("v", Int value) ]
  | Instant ->
     Trace.write_duration_instant
       trace.file ~args:[] ~thread ~category ~name ~time
  | _ -> ()
