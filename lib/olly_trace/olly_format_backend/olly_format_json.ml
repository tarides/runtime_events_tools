...
type trace = { file : out_channel; mutable first : bool }
...
let[@inline] array_elt_begin trace =
  if trace.first then trace.first <- false else Printf.fprintf trace.file ","

let close trace =
  output_char trace.file ']';
